...
export const questionInputSchema = z.union([
  ...
  z
    ...
    .refine(
      (q) => q.correctAnswer.every((id) => q.options.some((o) => o.id === id)),
      {
        message: "All correct answers must match given options.",
        path: ["correctAnswer"],
      },
    ),
  ...
  z
    ...
    .refine(
      (q) => {
        const paired = new Set(q.correctAnswer.map((p) => p.leftId));
        return q.options.left.every((l) => paired.has(l.id));
      },
      {
        message: "Every left item must have a matching pair.",
        path: ["correctAnswer"],
      },
    ),
  ...
  z
    ...
    .refine(
      (q) =>
        (q.prompt.match(/\{\{\d+\}\}/g)?.length ?? 0) ===
        q.correctAnswer.length,
      {
        message:
          "The number of {{n}} blanks in the prompt must match the number of answers.",
        path: ["correctAnswer"],
      },
    ),
